...
public class TetrisDrawingPanel extends JPanel implements Observer {
    ...
    /**
     * Static Field for Color forest Green.
     */
    private static final Color COLOR_FOREST = new Color(45, 128, 20);
    
    /**
     * Static Field for Color Brown.
     */
    private static final Color COLOR_BROWN = new Color(62, 75, 71);
    
    /**
     * Static Field for Color Brown.
     */
    private static final Color COLOR_GUNGRAY = new Color(135, 128, 123);
    ...
    protected void paintComponent(final Graphics theGraphics) {
        ...
        for (final Rectangle2D r : myDrawableBlocks) {
            final int smallOffeset = 30;
            final Point2D blockStart = new Point2D.Double(r.getX(), r.getY() + smallOffeset);
            final Point2D blockEnd = new Point2D.Double(r.getX() + smallOffeset, r.getY());
            final float[] blockDist = {0.0f, 1.0f};
            final Color[] blockColors = {COLOR_BROWN, COLOR_FOREST};
            final LinearGradientPaint blockLinearGradient = 
                        new LinearGradientPaint(blockStart, blockEnd, blockDist, blockColors);
            g2D.setPaint(blockLinearGradient);
            g2D.fill(r);
        ...
        if (!myFrozenBlocks.isEmpty()) {
            for (final Rectangle2D r2 : myFrozenBlocks) {
                final int offset = 10;
                final Point2D frozenStart = new Point2D.Double(r2.getX(), r2.getY() + offset);
                final Point2D frozenEnd = new Point2D.Double(r2.getX() + offset, r2.getY());
                final float[] frozenDist = {0.0f, 1.0f};
                final Color[] frozenColors = {COLOR_GUNGRAY, Color.BLACK};
                final LinearGradientPaint frozenLinearGradient = 
                                new LinearGradientPaint(frozenStart, 
                                                        frozenEnd, frozenDist, frozenColors);
                g2D.setPaint(frozenLinearGradient);
                g2D.fill(r2);
                g2D.setStroke(new BasicStroke(DEFAULT_STROKE));
