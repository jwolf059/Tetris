...
public final class Board extends Observable {
    ...
    /** 
     * Determines if the user activated the bomb. 
     */
    private boolean myBombActive;
    
    /**
     * Field to determine if the bomb piece is falling.
     */
    private boolean myBombDropping;
    
    /**
     * Field to determine if the bomb has exploded.
     */
    ...
    public void newGame(final int theWidth,
        ...
        myWidth = theWidth;
        myHeight = theHeight;
        // Sets the bomb active to false. Will be changed to true when the user 
        // activates their bomb.
        myBombActive = false;
        
        // Determines if the Bomb is actively falling.
        myBombDropping = false;
        
        // Determines if the Bomb is exploding.
    ...
    private void assignCurrentPiece() {
        ...
        // Sets the Bomb dropping boolean to true if the current piece is a
        // Bomb Piece.
        myBombDropping = ((AbstractPiece) myCurrentPiece).getBlock() == Block.B;
        assignNextPiece();
    }
     ...
     * specified coordinates. Unless the user has request the next piece
     * to be a Bomb.
     * 
    ...
    private Piece randomPiece(final int theX, final int theY) {
        ...
        Piece result;
        // Checks to see if the user has request a bomb as the next piece.
        if (myBombActive) {
            result = new BPiece(theX, theY);
            myBombActive = false;
        } else {
    ...
    private void freeze() {
        ...
        if (((AbstractPiece) myCurrentPiece).getBlock() == Block.B) {
            blowUp();
        }
        setChanged();
        notifyObservers();
    ...
    /**
     * Sets the myBombActive field to True, resulting in the next pieces selected 
     * to be the Bomb Tetris Piece.
     */
    ...
    /**
     * Getter method for the myBombExploded value.
     * 
     * @return the boolean value store in myBombExploded
     */
    public boolean isBombExploded() {
        return myBombExploded;
    }
    
    /**
     * Setter method for the boolean field myBombExploded.
     */
    ...
    /**
     * Used when a bomb piece is frozen. Determines how many lines need to be cleared
     * and which lines should be cleared. The maximum number of lines cleared will be
     * 7, this includes the 3 lines that the bomb blocks occupy.
     * 
     */
    public void blowUp() {
        setBombExploded();
        ...
        final int yCoord = ((AbstractPiece) myCurrentPiece).getY();
        if (yCoord >= four) {
            linesDestroyed = four;
            
        } else if (yCoord == three) {
            linesDestroyed = three;
        ...
        int val = yCoord - linesDestroyed;

        for (; val <= yCoord + 2; val++) {
            final Block[] row = myFrozenBlocks.get(val);
            for (int xVal = 0; xVal < row.length; xVal++) {
                row[xVal] = ((AbstractPiece) myCurrentPiece).getBlock();
